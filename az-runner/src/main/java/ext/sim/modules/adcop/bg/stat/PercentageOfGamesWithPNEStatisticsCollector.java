...
@Register(name = "ppne-sc")
public class PercentageOfGamesWithPNEStatisticsCollector extends AbstractStatisticCollector<PercentageOfGamesWithPNEStatisticsCollector.PercentageOfGamesWithPNEStatisticsCollectorRecord> {
	
    @Override
    public VisualModel analyze(Database db, Test r) {
    	String query = "select SUM(value) as sum, rVar, algorithm_instance from PercentageOfGamesWithPNEStatistics where TEST = '" + r.getName() + "' group by algorithm_instance, rVar";

    	//THEN YOU SHOULD CREATE A GRAPH TO REPRESENT THE QUERY IN LIKE THIS:
    	LineVisualModel line = new LineVisualModel(r.getRunningVarName(), "Percentage of Games with PNE", "Percentage of games that have a PNE without any taxation scheme");
    	...
    	try {
            ...
            Logger.getLogger(PercentageOfGamesWithPNEStatisticsCollector.class.getName()).log(Level.SEVERE, null, ex);
        }
        return null;
    }
    ...
    public void hookIn(final Agent[] agents, final Execution ex) {
    	ex.hookIn(new Hooks.TerminationHook() {
            ...
            public void hook() {
            	...
            	submit(new PercentageOfGamesWithPNEStatisticsCollectorRecord(ex.getTest().getCurrentVarValue(), utils.getOverallIncentiveCost(ex.getResult().getAssignment()) < utils.getMinMaxCostValue() ? 1 : 0));            	
            }
        });
    }
    ...
    public String getName() {
    	...
    	return "PercentageOfGamesWithPNEStatisticsCollector";
    }

    //THIS IS YOUR RECORD IN THE DATABASE DEFINITION
    public static class PercentageOfGamesWithPNEStatisticsCollectorRecord extends DBRecord {
        ...
        public PercentageOfGamesWithPNEStatisticsCollectorRecord(double rVar, int value) {
            this.rVar = rVar;
            this.value = value;
        }
        ...
        public String provideTableName() {
            return "PercentageOfGamesWithPNEStatistics";
        }
    }
}
